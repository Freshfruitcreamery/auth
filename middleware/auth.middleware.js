...
const resetPasswordSchemaAuth = Joi.object({
  password: Joi.string().required(),
...
exports.isPasswordInputValidated = async (req, res, next) => {
  const result = resetPasswordSchemaAuth.validate(req.body);
  // console.log(result)
  if ('error' in result) {
    res.status(400).json({
      error: 1,
 ...
 * @param {*} next
 */
exports.isPasswordInputValidated2 = async (req, res, next) => {
  const result = resetPasswordSchemaAuth.validate(req.body);
  // console.log(result)
  if ('error' in result) {
    res.status(400).json({
...
exports.verifyPasswordResetOtp = async (req, res, next) => {
  ...
  if (typeof bearerHeader !== 'undefined') {
    ...
    try {
      const decrypted_data = jwt.verify(
        bearerToken,
        process.env.private_sso_key
      );
